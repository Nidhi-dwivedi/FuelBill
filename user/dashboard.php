...
$total_transactions = $conn->query("SELECT COUNT(*) FROM transactions WHERE user_id = $user_id")->fetchColumn();
$total_spent = $conn->query("SELECT SUM(amount) FROM transactions WHERE user_id = $user_id")->fetchColumn();
$last_transaction = $conn->query("SELECT * FROM transactions WHERE user_id = $user_id ORDER BY transaction_date DESC LIMIT 1")->fetch(PDO::FETCH_ASSOC);
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <div class="flex h-screen">
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-100 p-6">
                <div class="mb-6">
                    <h2 class="text-2xl font-semibold">Welcome, <?php echo $_SESSION['full_name']; ?></h2>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                    ...
                    <div class="bg-white rounded-lg shadow p-6">
                        <div class="flex items-center">
                            ...
                            <div>
                                ...
                                <h3 class="text-2xl font-bold"><?php echo $total_transactions; ?></h3>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-lg shadow p-6">
                        <div class="flex items-center">
                            ...
                            <div>
                                ...
                                <h3 class="text-2xl font-bold">₹<?php echo number_format((float)$total_spent, 2); ?></h3>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-lg shadow p-6">
                        <div class="flex items-center">
                            ...
                            <div>
                                <p class="text-gray-500">Last Transaction</p>
                                <h3 class="text-2xl font-bold">
                                    <?php echo $last_transaction ? '₹' . number_format($last_transaction['amount'], 2) : 'N/A'; ?>
                                </h3>
                            </div>
                        </div>
                    </div>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                ...
                                while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                    echo "<tr>
                                        ...
                                        <td class='px-6 py-4 whitespace-nowrap text-sm text-gray-500'>" . ucfirst($row['transaction_type']) . "</td>
                                        <td class='px-6 py-4 whitespace-nowrap text-sm text-gray-500'>" . (isset($row['fuel_name']) ? $row['fuel_name'] : 'N/A') . "</td>
                                        <td class='px-6 py-4 whitespace-nowrap text-sm text-gray-500'>" . (isset($row['liters']) && $row['liters'] ? $row['liters'] . ' L' : 'N/A') . "</td>
                                        <td class='px-6 py-4 whitespace-nowrap text-sm text-gray-500'>₹{$row['amount']}</td>
                                        <td class='px-6 py-4 whitespace-nowrap text-sm text-gray-500'>" . date('M d, Y H:i', strtotime($row['transaction_date'])) . "</td>
                                    </tr>";
                                }
